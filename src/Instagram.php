...
class Instagram {
  ...
  public function searchUser($name, $limit = 0) {
    return $this->_makeCall('users/search', isset($this->_accesstoken), array('q' => $name, 'count' => $limit));
  }
   ...
   * @return mixed
   */
  public function getUserMedia($id = 'self', $limit = 0, $next_max_id = null) {
    return $this->_makeCall('users/' . $id . '/media/recent', strlen($this->getAccessToken()), array('count' => $limit, 'max_id' => $next_max_id));
  }
  ...
  public function searchMedia($lat, $lng, $distance = 1000, $minTimestamp = NULL, $maxTimestamp = NULL) {
    return $this->_makeCall('media/search', isset($this->_accesstoken), array('lat' => $lat, 'lng' => $lng, 'distance' => $distance, 'min_timestamp' => $minTimestamp, 'max_timestamp' => $maxTimestamp));
  }
